...
class Trainer:
    ...
    def train(self, train_loader, val_loader=None):
        ...
        for epoch in range(self.config.num_epochs):
            ...
            # Train
            train_metrics = self._train_epoch(train_loader, criterion, compute_gt_plausibility)
    ...
    def _train_epoch(self, train_loader, criterion, compute_gt_plausibility):
        """Train for one epoch"""
        self.model.train()
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            gt_plausibility = compute_gt_plausibility(
                descriptions, batch["constraints"]
            ).to(self.device)
                    ...
                    wandb.log(
                        {
                            ...
                            "train/verification_loss": loss_dict[
                                "verification_loss"
                            ].item(),
    ...
    def _validate_epoch(self, val_loader, criterion):
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc="Validating"):
                ...
                _ = self.model.generate(img_t1, img_t2, questions)

                # TODO: Compute validation metrics

        return {"val/loss": total_loss / len(val_loader) if len(val_loader) > 0 else 0.0}
    ...
    def _save_checkpoint(self, epoch):
        ...
        checkpoint_path = os.path.join(
            self.config.output_dir, f"checkpoint_epoch{epoch + 1}.pt"
        )
