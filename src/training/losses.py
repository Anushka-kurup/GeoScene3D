...
class GeoCarLoss(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        if outputs["confidence"] is not None and "gt_plausibility" in targets:
            losses["verification_loss"] = F.binary_cross_entropy(
                outputs["confidence"], targets["gt_plausibility"]
            )
        else:
            losses["verification_loss"] = torch.tensor(
                0.0, device=outputs["change_mask"].device
            )
        ...
        if "process_attention" in outputs and "change_mask" in outputs:
            losses["change_loss"] = self.compute_change_attention_loss(
                outputs["process_attention"], outputs["change_mask"]
            )
        else:
            losses["change_loss"] = torch.tensor(
                0.0, device=outputs["change_mask"].device
            )
        ...
        if "process_stages" in targets:
            losses["process_loss"] = self.compute_process_ordering_loss(
                outputs.get("process_attention"), targets["process_stages"]
            )
        else:
            losses["process_loss"] = torch.tensor(
                0.0, device=outputs["change_mask"].device
            )
    ...
    def compute_change_attention_loss(self, attention_weights, change_mask):
        ...
        change_mask_flat = F.adaptive_avg_pool2d(
            change_mask.unsqueeze(1), output_size=(int(N**0.5), int(N**0.5))
        ).flatten(2)  # [B, 1, N]

        # Normalize
        change_mask_norm = change_mask_flat / (
            change_mask_flat.sum(dim=-1, keepdim=True) + 1e-6
        )
        ...
        loss = F.kl_div(
            (avg_attention + 1e-6).log(), change_mask_norm + 1e-6, reduction="batchmean"
        )
